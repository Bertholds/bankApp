...
public class UserController implements Initializable {
	...
	public void handleDeleteClick(ActionEvent event) throws Exception {
		...
		if (!users.isEmpty()) {
			if (MethodUtilitaire.confirmationDialog(event, "Confirm to delete selected user ?",
				...
				try {
					...
					e.printStackTrace();
					MethodUtilitaire.deleteNoPersonSelectedAlert("Violation d'intégrité", "Violation d'intégrité", "Veuillez au préalable supprimer le compte d'utilisateur appartenant à cet utilisateur");
				}
			}
		} else {
	...
	public void handleEditClick(ActionEvent event) {
		...
		try {
			...
			if (MethodUtilitaire.emptyValidation("Pseudo", getPseudo().isEmpty())) {
				...
				LoadDataOnTable();
				enableBtn();
				methodUtilitaire.LogFile("Edition d'utilisateur",
						updateUtilisateur.getPseudo(), MethodUtilitaire.deserializationUser());
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void handleUserAccountClick(ActionEvent event) {
		...
		stage.close();
		stageManager.switchSceneShowPreviousStageInitOwner(FxmlView.USERACCOUNT);
	}
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		for (Adherent adherent : adherents) {
			adherentsList
					.add(adherent.getIdentifiant().toString() + " " + adherent.getNom() + " " + adherent.getPrenom());
		}
